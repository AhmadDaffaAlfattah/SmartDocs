...
@push('styles')
    ...
    <style>
        .viewer-container {
            background: white;
            padding: 20px;
            /* margin: 20px; Removed margin to fit better in master layout content area */
            border-radius: 8px;
    ...
    <div class="viewer-container">
        <div class="viewer-header">
            ...
            <div class="viewer-controls">
                <a href="{{ route('engineering.index') }}" class="btn-back">← Kembali</a>
        ...
        @elseif(in_array($fileType, ['xls', 'xlsx', 'csv']) && $isExcel && $sheetsData)
            ...
            <script>
                ...
                function downloadEditedExcel() {
                    ...
                    fetch('/engineering/export-excel', {
                        method: 'POST',
                        headers: {
        ...
        @else
            <div style="padding: 40px; text-align: center; color: #999;">
                ...
                <a href="{{ route('engineering.download', $document->id) }}" class="btn-download" style="display: inline-block; margin-top: 20px;">📥 Download File</a>
            </div>
        @endif
...
<script>
    ...
    const getFileUrl = '/engineering/' + documentId + '/get-file';
    const isExcel = {{ $isExcel ? 'true' : 'false' }};
    // sheetsData constant is already defined in the Excel block logic if isExcel is true
    // But we might need it for standard Handsontable if not using the custom editor
    ...
    function downloadFile() {
        // Check if Excel with edits/new rows
        ...
        // Download original file (PDF, Excel, or any other)
        window.location.href = '{{ route('engineering.download', $document->id) }}';
    }
</script>
@endpush
