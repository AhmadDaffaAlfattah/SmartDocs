...
@section('content')
    ...
    <div class="filter-section">
        ...
        <div class="search-box" style="margin-left: 15px;">
            <form method="GET" action="{{ route('engineering.index') }}" style="display: flex; align-items: center;">
                    ...
                    <tbody>
                        @foreach ($documents as $key => $document)
                            <tr>
                                ...
                                <td>{{ $document->getFolderBreadcrumb() }}</td>
                                ...
                                <td class="action">
                                    @if ($document->file_path)
                                        <a href="{{ route('engineering.viewer', $document->id) }}" class="action-btn" title="View" target="_blank">
                                    ...
                                    @if(auth()->user()->role !== 'user')
                                        <a href="{{ route('engineering.edit', $document->id) }}" class="action-btn" title="Edit">
                                            <img src="https://cdn-icons-png.flaticon.com/128/14034/14034493.png" alt="Edit" width="24" height="24">
                                        </a>
                                        <form id="delete-form-{{ $document->id }}" action="{{ route('engineering.destroy', $document->id) }}" method="POST" style="display: inline;" onsubmit="event.preventDefault(); confirmDelete('delete-form-{{ $document->id }}');">
                                            @csrf
                                            @method('DELETE')
