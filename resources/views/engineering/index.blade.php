...
@section('title', 'SmartDocs - Engineering')
...
@section('content')
    ...
    <div class="engineering-page-header">
        <div class="engineering-page-title">
            <div style="font-size: 28px; font-weight: bold; color: #333333; margin-bottom: 20px;">
                Document <span style="font-weight: bold;">» Engineering</span>
            </div>
            <a href="{{ route('engineering.create') }}" class="btn-tambah-data">
                ➕ Tambah Data
            </a>
        </div>
    </div>
    ...
    <div class="filter-section">
        ...
        <div class="search-box">
            <form method="GET" action="{{ route('engineering.index') }}" style="display: flex; width: 100%;">
                    ...
                    <tbody>
                        @foreach ($documents as $key => $document)
                            <tr>
                                ...
                                <td>
                                    <span title="{{ $document->getFolderBreadcrumb() }}">
                                ...
                                <td class="action">
                                    @if ($document->file_path)
                                        <a href="{{ route('engineering.viewer', $document->id) }}" class="action-btn view" title="View" target="_blank">
                                            <img src="{{ asset('images/view.png') }}" alt="View" title="View" width="32" height="32">
                                        </a>
                                    @elseif ($document->link)
                                    ...
                                    <form action="{{ route('engineering.destroy', $document->id) }}" method="POST" 
...
<script>
    ...
    function openEditModal(documentId) {
        ...
        fetch(`/engineering/${documentId}/get-edit-data`)
            ...
            .then(data => {
                ...
                if (data.document.file_path) {
                    fileInfo = `<strong>File saat ini:</strong> <a href="/engineering/${data.document.id}/download">${data.document.file_name}</a>`;
                }
