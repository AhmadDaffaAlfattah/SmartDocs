...
@section('title', 'SmartDocs - Edit Dokumen Operasi')
...
@section('content')
    <div class="engineering-page-header">
        <div class="engineering-page-title">
            <div style="font-size: 28px; font-weight: bold; color: #333;">
                Operasi <span style="font-weight: bold;">» Edit Dokumen</span>
            </div>
        </div>
    </div>
    ...
    <div style="background: white; padding: 30px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); max-width: 800px; margin: 30px auto 0;">
        <form action="{{ route('operasi.update', $document->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
...
@push('scripts')
<script>
    // Copy the tree logic but initialize selected properly
