...
@section('title', 'SmartDocs - Edit Dokumen Business Support')
...
@section('content')
    <div class="engineering-page-header">
        <div class="engineering-page-title">
            <div style="font-size: 28px; font-weight: bold; color: #333;">
                Business Support <span style="font-weight: bold;">» Edit Dokumen</span>
            </div>
        </div>
    </div>
    ...
    <div style="background: white; padding: 30px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); max-width: 800px; margin: 30px auto 0;">
        <form action="{{ route('business-support.update', $document->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="hidden" id="folder_id" name="folder_id" value="{{ old('folder_id', $document->folder_id) }}">
                @error('folder_id') <p style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</p> @enderror
            </div>

            <!-- File -->
            <div style="margin-bottom: 20px;">
                <label for="file" style="display: block; font-weight: 600; color: #333; margin-bottom: 8px;">Upload File Baru (Opsional)</label>
                <input type="file" id="file" name="file" style="width: 100%; padding: 10px 12px; border: 1px solid #999; border-radius: 4px;">
                <p style="color: #666; font-size: 12px; margin-top: 5px;">File saat ini: {{ $document->file_name ?? 'Tidak ada file' }}</p>
            ...
            <!-- Link -->
            <div style="margin-bottom: 20px;">
                <label for="link" style="display: block; font-weight: 600; color: #333; margin-bottom: 8px;">Update Link (Opsional)</label>
            ...
            <div style="display: flex; gap: 12px; margin-top: 30px;">
                <button type="submit" style="background-color: #333; color: white; border: none; padding: 10px 24px; border-radius: 4px; cursor: pointer; font-weight: 600;">Simpan Perubahan</button>
                <a href="{{ route('business-support.index') }}" style="background-color: #999; color: white; border: none; padding: 10px 24px; border-radius: 4px; text-decoration: none; font-weight: 600;">Batal</a>
            </div>
        </form>
    </div>
@endsection
...
<script>
    ...
    const currentFolderId = {{ old('folder_id', $document->folder_id) ?? 'null' }};
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const folderTreeHtml = renderFolderTree(folderTreeData, currentFolderId);
